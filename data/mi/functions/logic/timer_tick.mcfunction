...
execute if score $gameStage temp matches 2.. unless score $rushMode options matches 1 run scoreboard players operation #inflationTimer temp = $timer.Seconds timer
execute if score $gameStage temp matches 2.. unless score $rushMode options matches 1 run scoreboard players operation #inflationTimer temp %= 20 integers
execute if score $gameStage temp matches 2.. unless score $rushMode options matches 1 if score #inflationTimer temp matches 0 run function mi:logic/decrease_inflation
...
execute if score $gameStage temp matches 2.. as @a[team=!spectators,scores={timer.itemShop=10..}] run function mi:logic/item_shop
